...
output_file_path = 'BIS_debtrank/one_countries_g_c.xlsx'
...
for chunk in pd.read_csv(file_path, chunksize=chunksize):
    ...
    filtered_data = chunk[
        ...
        (chunk['L_MEASUREMeasure'] == 'G: Annual growth (BIS calculated)') &
...
if not final_filtered_rows.empty:
    ...
    aggregated_data = final_filtered_rows.groupby(
        ['TIME_PERIODTime period or range', 'L_REP_CTYReporting country', 'L_CP_COUNTRYCounterparty country']
    ).agg({'OBS_VALUEObservation Value': 'sum'}).reset_index()


    aggregated_data = pd.merge(final_filtered_rows.drop(columns='OBS_VALUEObservation Value'), aggregated_data, on=['TIME_PERIODTime period or range', 'L_REP_CTYReporting country', 'L_CP_COUNTRYCounterparty country'], how='right')
...
print(f"각 국가별로 다른 국가에 대한 OBS 값을 집계한 데이터와 필터링된 행이 '{output_file_path}' 파일에 저장되었습니다.")
