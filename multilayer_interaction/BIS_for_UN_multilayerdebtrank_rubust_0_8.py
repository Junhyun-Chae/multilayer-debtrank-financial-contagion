...
def run_multilayer(
    ...
    with pd.ExcelWriter(out_dir / "multilayer_results_BIS_0.8.xlsx") as writer:
        for per, hist in results_bis.items():
            pd.DataFrame(hist, columns=countries).to_excel(writer, sheet_name=f"{per.year}-Q{per.quarter}")
    with pd.ExcelWriter(out_dir / "multilayer_results_UN_0.8.xlsx") as writer:
...
def parse_args():
    ...
    p.add_argument("--vertical-threshold", type=float, default=0.8, help="수직 전이 임계값 θ")
...
##./output/multilayer_results_BIS_0.8.xlsx
##./output/multilayer_results_UN_0.8.xlsx