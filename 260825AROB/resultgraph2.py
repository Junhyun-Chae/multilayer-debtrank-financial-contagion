...
im = plt.imshow(norm_df.values, aspect="auto", cmap="viridis", interpolation="nearest")
cbar = plt.colorbar(im)
cbar.set_label("Normalized Final Risk", fontsize=30)
...
yticks = np.arange(0, len(x_labels), step)
ax.set_yticks(yticks)
ax.set_yticklabels([x_labels[i] for i in yticks], fontsize=25)

plt.xticks(
    ticks=np.arange(len(summary_df.columns)),
    labels=summary_df.columns,
    rotation=45, ha="right", fontsize=23
)
...
cbar = plt.colorbar(im)
cbar.set_label(r"Normalized Systemic Risk Score $\bar{h}_i(t)$", fontsize=30)
cbar.ax.tick_params(labelsize=25)                    
...
plt.title("Reproducing Historical Systemic Risk Episodes", fontsize=24, pad=25)
