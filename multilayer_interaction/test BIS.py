...
from mpl_toolkits.basemap import Basemap  # noqa: E402  pylint: disable=C0413
import matplotlib.animation as animation  # noqa: E402  pylint: disable=C0413
...
def agg_last(path):
    ...
    codes = [iso3(c) for c in xl.parse(xl.sheet_names[0]).columns[1:]]
    pos   = {c:i for i,c in enumerate(codes)}
    idx   = [pos[c] for c in order]    # 20개 인덱스
    s = np.zeros(len(codes)); cnt = np.zeros(len(codes),int)
    for sh in xl.sheet_names:
        last = xl.parse(sh).iloc[-1,1:].astype(float).values
        s += last; cnt += (last>=1.0)
